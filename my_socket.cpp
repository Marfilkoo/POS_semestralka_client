...
// takúto správu ":end" nesmiem nikde použivať v komunikaci, lebo mi to ukončí komunikáciu
// Keby to chcem zmiť stači tu dať "////" napríklad
...
MySocket* MySocket::createConnection(std::string hostName, short port) {
    ...
    if (connectSocket == INVALID_SOCKET) {
        WSACleanup(); // na prečistenie pamäte - lebo wsacstartup alouje miesto a ked to zlyha, musime to premazať
...
void MySocket::sendData(const std::string &data) {
    // 1 write neznamená 1 read - môže byť jeden write a 3 ready, môže byť 2 ready a možu byť 4 ready
    size_t data_length = data.length();
    char* buffer = (char*)calloc(data_length + 1, sizeof(char));
    memcpy(buffer, data.c_str(), data_length);
    buffer[data_length] = SOCKET_TERMINATE_CHAR; // na definvoanie, že správa končí /0 -> pre označenie konca správy
...
void MySocket::sendEndMessage() {
    // na ukočnenie komunikácie, keď nechcem poslieť správy
